import { expect, test, describe, beforeAll } from "bun:test";

import MOCK_USER_INFO from "./_mocks/userinfo_v1_3_0.json";
import { User } from "./game";
import { StarRailApi } from "./api";
import { StarRailService } from "./StarRailService.ts";
import { ApiCharacterData } from "./api/StarRailApi.type.ts";

// Since we have no tests under `./game`, we'll do E2E testing here
describe("StarRailService", async () => {
  let service: StarRailService;
  let users: User[];

  beforeAll(() => {
    users = [
      new User(
        StarRailService.fromApiPlayer(MOCK_USER_INFO.player),
        StarRailService.fromApiCharacters(
          MOCK_USER_INFO.characters as ApiCharacterData[],
        ),
        new StarRailApi(MOCK_USER_INFO.player.uid),
      ),
    ];
    service = new StarRailService(users);
  });

  describe("Users", () => {
    test("should throw error on API UUID mismatch", () => {
      expect(() => {
        new User(
          StarRailService.fromApiPlayer(MOCK_USER_INFO.player),
          StarRailService.fromApiCharacters(
            MOCK_USER_INFO.characters as ApiCharacterData[],
          ),
          new StarRailApi("123456"),
        );
      }).toThrow(
        "Fatal error: Player Api UUID mismatch when constructing User.",
      );
    });

    test("should initialize correctly", () => {
      expect(users).toBeArrayOfSize(1);
      expect(service.users).toEqual(users);
    });

    test("should return users", () => {
      expect(service.users).toStrictEqual(users);
    });

    test("should return user based on uuid", () => {
      expect(service.getUser(MOCK_USER_INFO.player.uid)).toStrictEqual(
        users[0],
      );
    });
  });

  describe("Player", () => {
    test("should parse Player data correctly", () => {
      const user = service.getUser(MOCK_USER_INFO.player.uid);
      if (!user) throw new Error("Critical error, user not found");
      const { player } = user;

      expect(player).toStrictEqual({
        avatar: {
          id: "202006",
          name: "Wanted Poster",
          icon: "icon/avatar/202006.png",
        },
        friendCount: 24,
        level: 67,
        nickname: "Atlas",
        signature: "you need arlan, trust me",
        spaceInfo: {
          achievementCount: 278,
          avatarCount: 24,
          challengeData: {
            mazeGroupId: 1,
            mazeGroupIndex: 21,
            preMazeGroupIndex: 1001,
          },
          lightConeCount: 62,
          passAreaProgress: 7,
        },
        uuid: "801875354",
        worldLevel: 6,
      });
    });
  });

  describe("Character", () => {
    test("should parse Character data correctly", () => {
      const user = service.getUser(MOCK_USER_INFO.player.uid);
      if (!user) throw new Error("Critical error, user not found");

      expect(user).toStrictEqual(users[0]);
      expect(user.characters).toEqual(EXPECTED_CHARACTERS);
    });
  });
});

// Super long & specific expected data, fold the code block please ^_^
const EXPECTED_CHARACTERS = [
  {
    id: "1008",
    name: "Arlan",
    rarity: 4,
    rank: 6,
    level: 80,
    promotion: 6,
    path: {
      id: "Warrior",
      name: "Destruction",
      icon: "icon/path/Destruction.png",
    },
    element: {
      id: "Thunder",
      name: "Lightning",
      color: "#8872F1",
      icon: "icon/element/Lightning.png",
    },
    skills: [
      {
        id: "100801",
        name: "Lightning Rush",
        level: 4,
        maxLevel: 9,
        element: {
          id: "Thunder",
          name: "Lightning",
          color: "#8872F1",
          icon: "icon/element/Lightning.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc: "Deals minor Lightning DMG to a single enemy.",
        desc: "Deals Lightning DMG equal to 80% of Arlan's ATK to a single enemy.",
        icon: "icon/skill/1008_basic_atk.png",
      },
      {
        id: "100802",
        name: "Shackle Breaker",
        level: 11,
        maxLevel: 15,
        element: {
          id: "Thunder",
          name: "Lightning",
          color: "#8872F1",
          icon: "icon/element/Lightning.png",
        },
        type: "BPSkill",
        typeText: "Skill",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc:
          "Consumes a portion of HP to deal Lightning DMG to a single enemy.",
        desc: "Consumes Arlan's HP equal to 15% of his Max HP to deal Lightning DMG equal to 252% of Arlan's ATK to a single enemy. If Arlan does not have sufficient HP, his HP will be reduced to 1 after using his Skill.",
        icon: "icon/skill/1008_skill.png",
      },
      {
        id: "100803",
        name: "Frenzied Punishment",
        level: 11,
        maxLevel: 15,
        element: {
          id: "Thunder",
          name: "Lightning",
          color: "#8872F1",
          icon: "icon/element/Lightning.png",
        },
        type: "Ultra",
        typeText: "Ultimate",
        effect: "Blast",
        effectText: "Blast",
        simpleDesc:
          "Deals massive Lightning DMG to a single enemy and Lightning DMG to enemies adjacent to it.",
        desc: "Deals Lightning DMG equal to 332% of Arlan's ATK to a single enemy and Lightning DMG equal to 166% of Arlan's ATK to enemies adjacent to it.",
        icon: "icon/skill/1008_ultimate.png",
      },
      {
        id: "100804",
        name: "Pain and Anger",
        level: 11,
        maxLevel: 15,
        element: null,
        type: "Talent",
        typeText: "Talent",
        effect: "Enhance",
        effectText: "Enhance",
        simpleDesc:
          "Increases DMG based on the current percentage of own HP lost.",
        desc: "Increases Arlan's DMG for every percent of HP below his Max HP, up to a max of 75% more DMG.",
        icon: "icon/skill/1008_talent.png",
      },
      {
        id: "100806",
        name: "100806",
        level: 0,
        maxLevel: 1,
        element: {
          id: "Thunder",
          name: "Lightning",
          color: "#8872F1",
          icon: "icon/element/Lightning.png",
        },
        type: "MazeNormal",
        typeText: "",
        effect: "MazeAttack",
        effectText: "",
        simpleDesc: "",
        desc: "",
        icon: "icon/skill/1008_basic_atk.png",
      },
      {
        id: "100807",
        name: "Swift Harvest",
        level: 1,
        maxLevel: 1,
        element: {
          id: "Thunder",
          name: "Lightning",
          color: "#8872F1",
          icon: "icon/element/Lightning.png",
        },
        type: "Maze",
        typeText: "Technique",
        effect: "MazeAttack",
        effectText: "",
        simpleDesc:
          "Attacks the enemy. After entering battle, deals minor Lightning DMG to all enemies.",
        desc: "Immediately attacks the enemy. After entering battle, deals Lightning DMG equal to 80% of Arlan's ATK to all enemies.",
        icon: "icon/skill/1008_technique.png",
      },
    ],
    skillTrees: [
      {
        id: "1008001",
        level: 3,
        anchor: "Point01",
        maxLevel: 6,
        icon: "icon/skill/1008_basic_atk.png",
        parent: null,
      },
      {
        id: "1008002",
        level: 9,
        anchor: "Point02",
        maxLevel: 10,
        icon: "icon/skill/1008_skill.png",
        parent: null,
      },
      {
        id: "1008003",
        level: 9,
        anchor: "Point03",
        maxLevel: 10,
        icon: "icon/skill/1008_ultimate.png",
        parent: null,
      },
      {
        id: "1008004",
        level: 9,
        anchor: "Point04",
        maxLevel: 10,
        icon: "icon/skill/1008_talent.png",
        parent: null,
      },
      {
        id: "1008007",
        level: 1,
        anchor: "Point05",
        maxLevel: 1,
        icon: "icon/skill/1008_technique.png",
        parent: null,
      },
      {
        id: "1008101",
        level: 1,
        anchor: "Point06",
        maxLevel: 1,
        icon: "icon/skill/1008_skilltree1.png",
        parent: null,
      },
      {
        id: "1008102",
        level: 1,
        anchor: "Point07",
        maxLevel: 1,
        icon: "icon/skill/1008_skilltree2.png",
        parent: null,
      },
      {
        id: "1008103",
        level: 1,
        anchor: "Point08",
        maxLevel: 1,
        icon: "icon/skill/1008_skilltree3.png",
        parent: null,
      },
      {
        id: "1008201",
        level: 1,
        anchor: "Point09",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: null,
      },
      {
        id: "1008202",
        level: 1,
        anchor: "Point10",
        maxLevel: 1,
        icon: "icon/property/IconStatusResistance.png",
        parent: "1008101",
      },
      {
        id: "1008203",
        level: 1,
        anchor: "Point11",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: "1008202",
      },
      {
        id: "1008204",
        level: 1,
        anchor: "Point12",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1008203",
      },
      {
        id: "1008205",
        level: 1,
        anchor: "Point13",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: "1008102",
      },
      {
        id: "1008206",
        level: 1,
        anchor: "Point14",
        maxLevel: 1,
        icon: "icon/property/IconStatusResistance.png",
        parent: "1008205",
      },
      {
        id: "1008207",
        level: 1,
        anchor: "Point15",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: "1008206",
      },
      {
        id: "1008208",
        level: 1,
        anchor: "Point16",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1008103",
      },
      {
        id: "1008209",
        level: 1,
        anchor: "Point17",
        maxLevel: 1,
        icon: "icon/property/IconStatusResistance.png",
        parent: "1008208",
      },
      {
        id: "1008210",
        level: 1,
        anchor: "Point18",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: "1008208",
      },
    ],
    equipment: {
      lightCone: {
        id: "1008",
        name: "Arlan",
        rarity: 4,
        rank: 6,
        level: 80,
        promotion: 6,
        icon: "icon/character/1008.png",
        preview: "image/character_preview/1008.png",
        portrait: "image/character_portrait/1008.png",
        path: {
          id: "Warrior",
          name: "Destruction",
          icon: "icon/path/Destruction.png",
        },
        attributes: [
          {
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 2257.92,
            display: "2257",
            percent: false,
          },
          {
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 1076.04,
            display: "1076",
            percent: false,
          },
          {
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 595.35,
            display: "595",
            percent: false,
          },
          {
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 102,
            display: "102",
            percent: false,
          },
          {
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.05,
            display: "5.0%",
            percent: true,
          },
          {
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.5,
            display: "50.0%",
            percent: true,
          },
        ],
        properties: [
          {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.755200007902463,
            display: "75.5%",
            percent: true,
          },
          {
            type: "StatusResistanceBase",
            field: "effect_res",
            name: "Effect RES",
            icon: "icon/property/IconStatusResistance.png",
            value: 0.41328000353649597,
            display: "41.3%",
            percent: true,
          },
          {
            type: "HPAddedRatio",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 0.37648000419140304,
            display: "37.6%",
            percent: true,
          },
          {
            type: "AllDamageTypeAddedRatio",
            field: "all_dmg",
            name: "DMG Boost",
            icon: "icon/property/IconAttack.png",
            value: 0.35,
            display: "35.0%",
            percent: true,
          },
          {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 1052.7679010287393,
            display: "1052",
            percent: false,
          },
          {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 471.3451300219167,
            display: "471",
            percent: false,
            count: 2,
            step: 2,
          },
          {
            type: "SpeedDelta",
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 29.93200000631623,
            display: "29",
            percent: false,
            count: 2,
            step: 3,
          },
          {
            type: "CriticalChanceBase",
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.647000004230065,
            display: "64.7%",
            percent: true,
            count: 4,
            step: 7,
          },
          {
            type: "CriticalDamageBase",
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.39528000773861893,
            display: "39.5%",
            percent: true,
            count: 1,
            step: 0,
          },
          {
            type: "DefenceDelta",
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 19.05189600144513,
            display: "19",
            percent: false,
            count: 1,
            step: 1,
          },
          {
            type: "BreakDamageAddedRatioBase",
            field: "break_dmg",
            name: "Break Effect",
            icon: "icon/property/IconBreakUp.png",
            value: 0.051840000785887,
            display: "5.1%",
            percent: true,
            count: 1,
            step: 0,
          },
          {
            type: "DefenceAddedRatio",
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 0.151199999498202,
            display: "15.1%",
            percent: true,
            count: 2,
            step: 4,
          },
          {
            type: "ThunderAddedRatio",
            field: "lightning_dmg",
            name: "Lightning DMG Boost",
            icon: "icon/property/IconThunderAddedRatio.png",
            value: 0.48880301429889395,
            display: "48.8%",
            percent: true,
          },
        ],
      },
      relics: [
        {
          id: "61091",
          name: "Band's Polarized Sunglasses",
          setId: "109",
          setName: "Band of Sizzling Thunder",
          rarity: 5,
          level: 15,
          icon: "icon/relic/109_0.png",
          mainAffix: {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 705.6000000101048,
            display: "705",
            percent: false,
          },
          subAffix: [
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 38.10379200289026,
              display: "38",
              percent: false,
              count: 2,
              step: 2,
            },
            {
              type: "SpeedDelta",
              field: "spd",
              name: "SPD",
              icon: "icon/property/IconSpeed.png",
              value: 4.900000000838191,
              display: "4",
              percent: false,
              count: 2,
              step: 3,
            },
            {
              type: "CriticalChanceBase",
              field: "crit_rate",
              name: "CRIT Rate",
              icon: "icon/property/IconCriticalChance.png",
              value: 0.126360003137963,
              display: "12.6%",
              percent: true,
              count: 4,
              step: 7,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.051840000785887,
              display: "5.1%",
              percent: true,
              count: 1,
              step: 0,
            },
          ],
        },
        {
          id: "61092",
          name: "Band's Touring Bracelet",
          setId: "109",
          setName: "Band of Sizzling Thunder",
          rarity: 5,
          level: 15,
          icon: "icon/relic/109_1.png",
          mainAffix: {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 352.8000000116881,
            display: "352",
            percent: false,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 105.8438720041886,
              display: "105",
              percent: false,
              count: 3,
              step: 1,
            },
            {
              type: "DefenceDelta",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 19.05189600144513,
              display: "19",
              percent: false,
              count: 1,
              step: 1,
            },
            {
              type: "HPAddedRatio",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 0.077760001178832,
              display: "7.7%",
              percent: true,
              count: 2,
              step: 2,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.168480003252625,
              display: "16.8%",
              percent: true,
              count: 3,
              step: 2,
            },
          ],
        },
        {
          id: "61093",
          name: "Band's Leather Jacket With Studs",
          setId: "109",
          setName: "Band of Sizzling Thunder",
          rarity: 5,
          level: 15,
          icon: "icon/relic/109_2.png",
          mainAffix: {
            type: "CriticalChanceBase",
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.32399999862536205,
            display: "32.3%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 42.337549002841115,
              display: "42",
              percent: false,
              count: 1,
              step: 2,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.11016000201925599,
              display: "11.0%",
              percent: true,
              count: 2,
              step: 1,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.159840002423155,
              display: "15.9%",
              percent: true,
              count: 4,
              step: 5,
            },
            {
              type: "BreakDamageAddedRatioBase",
              field: "break_dmg",
              name: "Break Effect",
              icon: "icon/property/IconBreakUp.png",
              value: 0.051840000785887,
              display: "5.1%",
              percent: true,
              count: 1,
              step: 0,
            },
          ],
        },
        {
          id: "61094",
          name: "Band's Ankle Boots With Rivets",
          setId: "109",
          setName: "Band of Sizzling Thunder",
          rarity: 5,
          level: 15,
          icon: "icon/relic/109_3.png",
          mainAffix: {
            type: "SpeedDelta",
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 25.03200000547804,
            display: "25",
            percent: false,
          },
          subAffix: [
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 38.10379200289026,
              display: "38",
              percent: false,
              count: 2,
              step: 2,
            },
            {
              type: "AttackAddedRatio",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 0.043200000654907,
              display: "4.3%",
              percent: true,
              count: 1,
              step: 2,
            },
            {
              type: "DefenceAddedRatio",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 0.107999999541788,
              display: "10.7%",
              percent: true,
              count: 2,
              step: 4,
            },
            {
              type: "CriticalChanceBase",
              field: "crit_rate",
              name: "CRIT Rate",
              icon: "icon/property/IconCriticalChance.png",
              value: 0.11664000246674,
              display: "11.6%",
              percent: true,
              count: 4,
              step: 4,
            },
          ],
        },
        {
          id: "63065",
          name: "Salsotto's Moving City",
          setId: "306",
          setName: "Inert Salsotto",
          rarity: 5,
          level: 15,
          icon: "icon/relic/306_0.png",
          mainAffix: {
            type: "ThunderAddedRatio",
            field: "lightning_dmg",
            name: "Lightning DMG Boost",
            icon: "icon/property/IconThunderAddedRatio.png",
            value: 0.38880301429889397,
            display: "38.8%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 122.7788920076564,
              display: "122",
              percent: false,
              count: 3,
              step: 5,
            },
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 42.337546004448086,
              display: "42",
              percent: false,
              count: 2,
              step: 4,
            },
            {
              type: "HPAddedRatio",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 0.043200000654907,
              display: "4.3%",
              percent: true,
              count: 1,
              step: 2,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.07344000111334101,
              display: "7.3%",
              percent: true,
              count: 2,
              step: 1,
            },
          ],
        },
        {
          id: "63066",
          name: "Salsotto's Terminator Line",
          setId: "306",
          setName: "Inert Salsotto",
          rarity: 5,
          level: 15,
          icon: "icon/relic/306_1.png",
          mainAffix: {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.43200000724755605,
            display: "43.2%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 76.20758800394833,
              display: "76",
              percent: false,
              count: 2,
              step: 2,
            },
            {
              type: "HPAddedRatio",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 0.155520002357664,
              display: "15.5%",
              percent: true,
              count: 4,
              step: 4,
            },
            {
              type: "DefenceAddedRatio",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 0.043199999956414,
              display: "4.3%",
              percent: true,
              count: 1,
              step: 0,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.064800001680851,
              display: "6.4%",
              percent: true,
              count: 1,
              step: 2,
            },
          ],
        },
      ],
    },
  },
  {
    id: "1209",
    name: "Yanqing",
    rarity: 5,
    rank: 1,
    level: 80,
    promotion: 6,
    path: {
      id: "Rogue",
      name: "The Hunt",
      icon: "icon/path/Hunt.png",
    },
    element: {
      id: "Ice",
      name: "Ice",
      color: "#47C7FD",
      icon: "icon/element/Ice.png",
    },
    skills: [
      {
        id: "120901",
        name: "Frost Thorn",
        level: 4,
        maxLevel: 9,
        element: {
          id: "Ice",
          name: "Ice",
          color: "#47C7FD",
          icon: "icon/element/Ice.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc: "Deals minor Ice DMG to a single enemy.",
        desc: "Deals Ice DMG equal to 80% of Yanqing's ATK to a single enemy.",
        icon: "icon/skill/1209_basic_atk.png",
      },
      {
        id: "120902",
        name: "Darting Ironthorn",
        level: 8,
        maxLevel: 15,
        element: {
          id: "Ice",
          name: "Ice",
          color: "#47C7FD",
          icon: "icon/element/Ice.png",
        },
        type: "BPSkill",
        typeText: "Skill",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc:
          "Deals Ice DMG to a single enemy and activates the Soulsteel Sync.",
        desc: "Deals Ice DMG equal to 192% of Yanqing's ATK to a single enemy and activates Soulsteel Sync for 1 turn.",
        icon: "icon/skill/1209_skill.png",
      },
      {
        id: "120903",
        name: "Amidst the Raining Bliss",
        level: 8,
        maxLevel: 15,
        element: {
          id: "Ice",
          name: "Ice",
          color: "#47C7FD",
          icon: "icon/element/Ice.png",
        },
        type: "Ultra",
        typeText: "Ultimate",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc:
          'Increases Yanqing\'s CRIT Rate. Enhances "Soulsteel Sync" and deals massive Ice DMG to a single enemy.',
        desc: "Increases Yanqing's CRIT Rate by 60%. When Soulsteel Sync is active, increases Yanqing's CRIT DMG by an extra 45%. This buff lasts for one turn. Afterwards, deals Ice DMG equal to 315% of Yanqing's ATK to a single enemy.",
        icon: "icon/skill/1209_ultimate.png",
      },
      {
        id: "120904",
        name: "One With the Sword",
        level: 8,
        maxLevel: 15,
        element: {
          id: "Ice",
          name: "Ice",
          color: "#47C7FD",
          icon: "icon/element/Ice.png",
        },
        type: "Talent",
        typeText: "Talent",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc:
          "During Soulsteel Sync, reduces the chance of this character being attacked and increases their CRIT Rate and CRIT DMG. After attacking an enemy, there is a chance of launching a follow-up attack, dealing Ice DMG with a chance to Freeze the target.\nSoulsteel Sync will be removed after this character receives damage.",
        desc: "When Soulsteel Sync is active, Yanqing is less likely to be attacked by enemies. Yanqing's CRIT Rate increases by 18.8% and his CRIT DMG increases by 26%. After Yanqing attacks an enemy, there is a 57% fixed chance to perform a follow-up attack, dealing Ice DMG equal to 43% of Yanqing's ATK to the enemy, which has a 65% base chance to Freeze the enemy for 1 turn.\nThe Frozen target cannot take action and receives Additional Ice DMG equal to 43% of Yanqing's ATK at the beginning of each turn.\nWhen Yanqing receives DMG, the Soulsteel Sync effect will disappear.",
        icon: "icon/skill/1209_talent.png",
      },
      {
        id: "120906",
        name: "120906",
        level: 0,
        maxLevel: 1,
        element: {
          id: "Ice",
          name: "Ice",
          color: "#47C7FD",
          icon: "icon/element/Ice.png",
        },
        type: "MazeNormal",
        typeText: "",
        effect: "MazeAttack",
        effectText: "",
        simpleDesc: "",
        desc: "",
        icon: "icon/skill/1209_basic_atk.png",
      },
      {
        id: "120907",
        name: "The One True Sword",
        level: 1,
        maxLevel: 1,
        element: null,
        type: "Maze",
        typeText: "Technique",
        effect: "Enhance",
        effectText: "Enhance",
        simpleDesc:
          "After this character uses Technique, at the start of the next battle, increases the DMG dealt by this character to enemy targets whose HP is 50% or higher.",
        desc: "After using his Technique, at the start of the next battle, Yanqing deals 30% more DMG for 2 turn(s) to enemies whose current HP is 50% or higher.",
        icon: "icon/skill/1209_technique.png",
      },
    ],
    skillTrees: [
      {
        id: "1209001",
        level: 4,
        anchor: "Point01",
        maxLevel: 6,
        icon: "icon/skill/1209_basic_atk.png",
        parent: null,
      },
      {
        id: "1209002",
        level: 8,
        anchor: "Point02",
        maxLevel: 10,
        icon: "icon/skill/1209_skill.png",
        parent: null,
      },
      {
        id: "1209003",
        level: 8,
        anchor: "Point03",
        maxLevel: 10,
        icon: "icon/skill/1209_ultimate.png",
        parent: null,
      },
      {
        id: "1209004",
        level: 8,
        anchor: "Point04",
        maxLevel: 10,
        icon: "icon/skill/1209_talent.png",
        parent: null,
      },
      {
        id: "1209007",
        level: 1,
        anchor: "Point05",
        maxLevel: 1,
        icon: "icon/skill/1209_technique.png",
        parent: null,
      },
      {
        id: "1209101",
        level: 1,
        anchor: "Point06",
        maxLevel: 1,
        icon: "icon/skill/1209_skilltree1.png",
        parent: null,
      },
      {
        id: "1209102",
        level: 1,
        anchor: "Point07",
        maxLevel: 1,
        icon: "icon/skill/1209_skilltree2.png",
        parent: null,
      },
      {
        id: "1209103",
        level: 1,
        anchor: "Point08",
        maxLevel: 1,
        icon: "icon/skill/1209_skilltree3.png",
        parent: null,
      },
      {
        id: "1209201",
        level: 1,
        anchor: "Point09",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: null,
      },
      {
        id: "1209202",
        level: 1,
        anchor: "Point10",
        maxLevel: 1,
        icon: "icon/property/IconIceAddedRatio.png",
        parent: "1209101",
      },
      {
        id: "1209203",
        level: 1,
        anchor: "Point11",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: "1209202",
      },
      {
        id: "1209204",
        level: 1,
        anchor: "Point12",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: null,
      },
      {
        id: "1209205",
        level: 1,
        anchor: "Point13",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: "1209102",
      },
      {
        id: "1209206",
        level: 1,
        anchor: "Point14",
        maxLevel: 1,
        icon: "icon/property/IconIceAddedRatio.png",
        parent: "1209205",
      },
      {
        id: "1209207",
        level: 1,
        anchor: "Point15",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: null,
      },
      {
        id: "1209208",
        level: 1,
        anchor: "Point16",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1209103",
      },
      {
        id: "1209209",
        level: 1,
        anchor: "Point17",
        maxLevel: 1,
        icon: "icon/property/IconIceAddedRatio.png",
        parent: "1209208",
      },
      {
        id: "1209210",
        level: 1,
        anchor: "Point18",
        maxLevel: 1,
        icon: "icon/property/IconAttack.png",
        parent: "1209208",
      },
    ],
    equipment: {
      lightCone: {
        id: "1209",
        name: "Yanqing",
        rarity: 5,
        rank: 1,
        level: 80,
        promotion: 6,
        icon: "icon/character/1209.png",
        preview: "image/character_preview/1209.png",
        portrait: "image/character_portrait/1209.png",
        path: {
          id: "Rogue",
          name: "The Hunt",
          icon: "icon/path/Hunt.png",
        },
        attributes: [
          {
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 1950.9840000000002,
            display: "1950",
            percent: false,
          },
          {
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 1261.26,
            display: "1261",
            percent: false,
          },
          {
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 875.385,
            display: "875",
            percent: false,
          },
          {
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 109,
            display: "109",
            percent: false,
          },
          {
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.05,
            display: "5.0%",
            percent: true,
          },
          {
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.5,
            display: "50.0%",
            percent: true,
          },
        ],
        properties: [
          {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 1.2606400162633602,
            display: "126.0%",
            percent: true,
          },
          {
            type: "IceAddedRatio",
            field: "ice_dmg",
            name: "Ice DMG Boost",
            icon: "icon/property/IconIceAddedRatio.png",
            value: 0.632803014298894,
            display: "63.2%",
            percent: true,
          },
          {
            type: "HPAddedRatio",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 0.1,
            display: "10.0%",
            percent: true,
          },
          {
            type: "CriticalDamageBase",
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 1.45144001398236,
            display: "145.1%",
            percent: true,
          },
          {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 866.4826860197354,
            display: "866",
            percent: false,
          },
          {
            type: "DefenceAddedRatio",
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 0.29159999918192403,
            display: "29.1%",
            percent: true,
            count: 3,
            step: 3,
          },
          {
            type: "StatusProbabilityBase",
            field: "effect_hit",
            name: "Effect Hit Rate",
            icon: "icon/property/IconStatusProbability.png",
            value: 0.28512000432238505,
            display: "28.5%",
            percent: true,
            count: 1,
            step: 2,
          },
          {
            type: "StatusResistanceBase",
            field: "effect_res",
            name: "Effect RES",
            icon: "icon/property/IconStatusResistance.png",
            value: 0.211680003209041,
            display: "21.1%",
            percent: true,
            count: 2,
            step: 0,
          },
          {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 547.5527140276972,
            display: "547",
            percent: false,
          },
          {
            type: "DefenceDelta",
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 71.97383000422269,
            display: "71",
            percent: false,
            count: 3,
            step: 2,
          },
          {
            type: "BreakDamageAddedRatioBase",
            field: "break_dmg",
            name: "Break Effect",
            icon: "icon/property/IconBreakUp.png",
            value: 0.051840000785887,
            display: "5.1%",
            percent: true,
            count: 1,
            step: 0,
          },
          {
            type: "SpeedDelta",
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 4.6000000005587935,
            display: "4",
            percent: false,
            count: 2,
            step: 2,
          },
        ],
      },
      relics: [
        {
          id: "61041",
          name: "Hunter's Artaius Hood",
          setId: "104",
          setName: "Hunter of Glacial Forest",
          rarity: 5,
          level: 15,
          icon: "icon/relic/104_0.png",
          mainAffix: {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 705.6000000101048,
            display: "705",
            percent: false,
          },
          subAffix: [
            {
              type: "DefenceAddedRatio",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 0.14579999959096202,
              display: "14.5%",
              percent: true,
              count: 3,
              step: 3,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.103680001571774,
              display: "10.3%",
              percent: true,
              count: 2,
              step: 0,
            },
            {
              type: "StatusProbabilityBase",
              field: "effect_hit",
              name: "Effect Hit Rate",
              icon: "icon/property/IconStatusProbability.png",
              value: 0.043200000654907,
              display: "4.3%",
              percent: true,
              count: 1,
              step: 2,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.06912000104785,
              display: "6.9%",
              percent: true,
              count: 2,
              step: 0,
            },
          ],
        },
        {
          id: "61042",
          name: "Hunter's Lizard Gloves",
          setId: "104",
          setName: "Hunter of Glacial Forest",
          rarity: 5,
          level: 15,
          icon: "icon/relic/104_1.png",
          mainAffix: {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 352.8000000116881,
            display: "352",
            percent: false,
          },
          subAffix: [
            {
              type: "DefenceDelta",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 55.038811003556475,
              display: "55",
              percent: false,
              count: 3,
              step: 2,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.187920004595071,
              display: "18.7%",
              percent: true,
              count: 3,
              step: 5,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.06912000104785,
              display: "6.9%",
              percent: true,
              count: 2,
              step: 0,
            },
            {
              type: "BreakDamageAddedRatioBase",
              field: "break_dmg",
              name: "Break Effect",
              icon: "icon/property/IconBreakUp.png",
              value: 0.051840000785887,
              display: "5.1%",
              percent: true,
              count: 1,
              step: 0,
            },
          ],
        },
        {
          id: "61043",
          name: "Hunter's Ice Dragon Cloak",
          setId: "104",
          setName: "Hunter of Glacial Forest",
          rarity: 5,
          level: 15,
          icon: "icon/relic/104_2.png",
          mainAffix: {
            type: "CriticalDamageBase",
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.648000007029628,
            display: "64.8%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 42.337549002841115,
              display: "42",
              percent: false,
              count: 1,
              step: 2,
            },
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 78.32446100655943,
              display: "78",
              percent: false,
              count: 4,
              step: 5,
            },
            {
              type: "DefenceDelta",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 16.935019000666216,
              display: "16",
              percent: false,
              count: 1,
              step: 0,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.07344000111334101,
              display: "7.3%",
              percent: true,
              count: 2,
              step: 1,
            },
          ],
        },
        {
          id: "61044",
          name: "Hunter's Soft Elkskin Boots",
          setId: "104",
          setName: "Hunter of Glacial Forest",
          rarity: 5,
          level: 15,
          icon: "icon/relic/104_3.png",
          mainAffix: {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.43200000724755605,
            display: "43.2%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 38.103794001974165,
              display: "38",
              percent: false,
              count: 1,
              step: 1,
            },
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 42.337546004448086,
              display: "42",
              percent: false,
              count: 2,
              step: 4,
            },
            {
              type: "SpeedDelta",
              field: "spd",
              name: "SPD",
              icon: "icon/property/IconSpeed.png",
              value: 4.6000000005587935,
              display: "4",
              percent: false,
              count: 2,
              step: 2,
            },
            {
              type: "StatusProbabilityBase",
              field: "effect_hit",
              name: "Effect Hit Rate",
              icon: "icon/property/IconStatusProbability.png",
              value: 0.11664000176824801,
              display: "11.6%",
              percent: true,
              count: 3,
              step: 3,
            },
          ],
        },
        {
          id: "63055",
          name: "Planet Screwllum's Mechanical Sun",
          setId: "305",
          setName: "Celestial Differentiator",
          rarity: 5,
          level: 15,
          icon: "icon/relic/305_0.png",
          mainAffix: {
            type: "IceAddedRatio",
            field: "ice_dmg",
            name: "Ice DMG Boost",
            icon: "icon/property/IconIceAddedRatio.png",
            value: 0.38880301429889397,
            display: "38.8%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 80.44134300481528,
              display: "80",
              percent: false,
              count: 2,
              step: 3,
            },
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 35.986915002111346,
              display: "35",
              percent: false,
              count: 2,
              step: 1,
            },
            {
              type: "AttackAddedRatio",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 0.11664000176824801,
              display: "11.6%",
              percent: true,
              count: 3,
              step: 3,
            },
            {
              type: "DefenceAddedRatio",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 0.053999999770894,
              display: "5.3%",
              percent: true,
              count: 1,
              step: 2,
            },
          ],
        },
        {
          id: "63056",
          name: "Planet Screwllum's Ring System",
          setId: "305",
          setName: "Celestial Differentiator",
          rarity: 5,
          level: 15,
          icon: "icon/relic/305_1.png",
          mainAffix: {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.43200000724755605,
            display: "43.2%",
            percent: true,
          },
          subAffix: [
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 38.10379200289026,
              display: "38",
              percent: false,
              count: 2,
              step: 2,
            },
            {
              type: "DefenceAddedRatio",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 0.09179999982006799,
              display: "9.1%",
              percent: true,
              count: 2,
              step: 1,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.051840000785887,
              display: "5.1%",
              percent: true,
              count: 1,
              step: 0,
            },
            {
              type: "StatusProbabilityBase",
              field: "effect_hit",
              name: "Effect Hit Rate",
              icon: "icon/property/IconStatusProbability.png",
              value: 0.12528000189923,
              display: "12.5%",
              percent: true,
              count: 3,
              step: 5,
            },
          ],
        },
      ],
    },
  },
  {
    id: "1205",
    name: "Blade",
    rarity: 5,
    rank: 0,
    level: 80,
    promotion: 6,
    path: {
      id: "Warrior",
      name: "Destruction",
      icon: "icon/path/Destruction.png",
    },
    element: {
      id: "Wind",
      name: "Wind",
      color: "#00FF9C",
      icon: "icon/element/Wind.png",
    },
    skills: [
      {
        id: "120501",
        name: "Shard Sword",
        level: 5,
        maxLevel: 9,
        element: {
          id: "Wind",
          name: "Wind",
          color: "#00FF9C",
          icon: "icon/element/Wind.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc: "Deals minor Wind DMG to an enemy.",
        desc: "Deals 90% of Blade's ATK as Wind DMG to a target enemy.",
        icon: "icon/skill/1205_basic_atk.png",
      },
      {
        id: "120502",
        name: "Hellscape",
        level: 8,
        maxLevel: 15,
        element: {
          id: "Wind",
          name: "Wind",
          color: "#00FF9C",
          icon: "icon/element/Wind.png",
        },
        type: "BPSkill",
        typeText: "Skill",
        effect: "Enhance",
        effectText: "Enhance",
        simpleDesc:
          "Consumes HP to Enhance Basic ATK, and this turn does not end after this Skill is used.",
        desc: "Consumes HP equal to 30% of Blade's Max HP to enter the Hellscape state.\nWhen Hellscape is active, his Skill cannot be used, his DMG dealt increases by 33%, and his Basic ATK Shard Sword is enhanced to Forest of Swords for 3 turn(s).\nIf Blade's current HP is insufficient, his HP will be reduced to 1 when he uses his Skill.\nThis Skill does not regenerate Energy. Using this Skill does not end the current turn.",
        icon: "icon/skill/1205_skill.png",
      },
      {
        id: "120503",
        name: "Death Sentence",
        level: 8,
        maxLevel: 15,
        element: {
          id: "Wind",
          name: "Wind",
          color: "#00FF9C",
          icon: "icon/element/Wind.png",
        },
        type: "Ultra",
        typeText: "Ultimate",
        effect: "Blast",
        effectText: "Blast",
        simpleDesc:
          "Sets current HP to 50% of Max HP, and deals massive Wind DMG to a single enemy and Wind DMG to adjacent targets.",
        desc: "Sets Blade's current HP to 50% of his Max HP and deals Wind DMG to a single enemy equal to the sum of 36% of his ATK, 90% of his Max HP, and 90% of the tally of Blade's HP loss in the current battle. At the same time, deals Wind DMG to adjacent targets equal to the sum of 14.4% of his ATK, 36% of his Max HP, and 36% of the tally of his HP loss in the current battle.\nThe tally of Blade's HP loss in the current battle is capped at 90% of his Max HP. This value will be reset and re-accumulated after his Ultimate has been used.",
        icon: "icon/skill/1205_ultimate.png",
      },
      {
        id: "120504",
        name: "Shuhu's Gift",
        level: 8,
        maxLevel: 15,
        element: {
          id: "Wind",
          name: "Wind",
          color: "#00FF9C",
          icon: "icon/element/Wind.png",
        },
        type: "Talent",
        typeText: "Talent",
        effect: "AoEAttack",
        effectText: "AoE",
        simpleDesc:
          "When Blade's HP is lowered, he gains 1 stack of Charge. When maximum Charge stack is reached, Blade immediately deals Wind DMG to all enemies and restores HP. Then, all Charges are consumed.",
        desc: "When Blade sustains DMG or consumes his HP, he gains 1 stack of Charge, stacking up to 5 times. A max of 1 Charge stack can be gained every time he is attacked.\nWhen Charge stack reaches maximum, immediately launches a follow-up attack on all enemies, dealing Wind DMG equal to 38% of Blade's ATK plus 96% of his Max HP. At the same time, restores Blade's HP by 25% of his Max HP. After the follow-up attack, all Charges are consumed.",
        icon: "icon/skill/1205_talent.png",
      },
      {
        id: "120506",
        name: "120506",
        level: 0,
        maxLevel: 1,
        element: {
          id: "Wind",
          name: "Wind",
          color: "#00FF9C",
          icon: "icon/element/Wind.png",
        },
        type: "MazeNormal",
        typeText: "",
        effect: "MazeAttack",
        effectText: "",
        simpleDesc: "",
        desc: "",
        icon: "icon/skill/1205_basic_atk.png",
      },
      {
        id: "120507",
        name: "Karma Wind",
        level: 1,
        maxLevel: 1,
        element: {
          id: "Wind",
          name: "Wind",
          color: "#00FF9C",
          icon: "icon/element/Wind.png",
        },
        type: "Maze",
        typeText: "Technique",
        effect: "MazeAttack",
        effectText: "",
        simpleDesc:
          "Attacks the enemy. After entering combat, consumes own HP and deals Wind DMG to all enemies.",
        desc: "Immediately attacks the enemy. After entering combat, consumes 20% of Blade's Max HP while dealing Wind DMG equal to 40% of his Max HP to all enemies.\nIf Blade's current HP is insufficient, his HP will be reduced to 1 when this Technique is used.",
        icon: "icon/skill/1205_technique.png",
      },
      {
        id: "120508",
        name: "Forest of Swords",
        level: 5,
        maxLevel: 9,
        element: {
          id: "Wind",
          name: "Wind",
          color: "#00FF9C",
          icon: "icon/element/Wind.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "Blast",
        effectText: "Blast",
        simpleDesc:
          "Consumes HP and deals Wind DMG to an enemy, as well as minor Wind DMG to adjacent targets.",
        desc: "Consumes HP equal to 10% of Blade's Max HP and deals Wind DMG equal to the sum of 36% of his ATK and 90% of his Max HP to a single enemy. In addition, deals Wind DMG equal to the sum of 14% of Blade's ATK and 36% of his Max HP to adjacent targets.\nIf Blade's current HP is insufficient, his HP will be reduced to 1 when using Forest of Swords.\nForest of Swords cannot regenerate Skill Points.",
        icon: "icon/skill/1205_basic_atk.png",
      },
    ],
    skillTrees: [
      {
        id: "1205001",
        level: 5,
        anchor: "Point01",
        maxLevel: 6,
        icon: "icon/skill/1205_basic_atk.png",
        parent: null,
      },
      {
        id: "1205002",
        level: 8,
        anchor: "Point02",
        maxLevel: 10,
        icon: "icon/skill/1205_skill.png",
        parent: null,
      },
      {
        id: "1205003",
        level: 8,
        anchor: "Point03",
        maxLevel: 10,
        icon: "icon/skill/1205_ultimate.png",
        parent: null,
      },
      {
        id: "1205004",
        level: 8,
        anchor: "Point04",
        maxLevel: 10,
        icon: "icon/skill/1205_talent.png",
        parent: null,
      },
      {
        id: "1205007",
        level: 1,
        anchor: "Point05",
        maxLevel: 1,
        icon: "icon/skill/1205_technique.png",
        parent: null,
      },
      {
        id: "1205101",
        level: 1,
        anchor: "Point06",
        maxLevel: 1,
        icon: "icon/skill/1205_skilltree1.png",
        parent: null,
      },
      {
        id: "1205102",
        level: 1,
        anchor: "Point07",
        maxLevel: 1,
        icon: "icon/skill/1205_skilltree2.png",
        parent: null,
      },
      {
        id: "1205103",
        level: 1,
        anchor: "Point08",
        maxLevel: 1,
        icon: "icon/skill/1205_skilltree3.png",
        parent: null,
      },
      {
        id: "1205201",
        level: 1,
        anchor: "Point09",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: null,
      },
      {
        id: "1205202",
        level: 1,
        anchor: "Point10",
        maxLevel: 1,
        icon: "icon/property/IconCriticalChance.png",
        parent: "1205101",
      },
      {
        id: "1205203",
        level: 1,
        anchor: "Point11",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1205202",
      },
      {
        id: "1205204",
        level: 1,
        anchor: "Point12",
        maxLevel: 1,
        icon: "icon/property/IconStatusResistance.png",
        parent: "1205203",
      },
      {
        id: "1205205",
        level: 1,
        anchor: "Point13",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1205102",
      },
      {
        id: "1205206",
        level: 1,
        anchor: "Point14",
        maxLevel: 1,
        icon: "icon/property/IconCriticalChance.png",
        parent: "1205205",
      },
      {
        id: "1205207",
        level: 1,
        anchor: "Point15",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1205206",
      },
      {
        id: "1205208",
        level: 1,
        anchor: "Point16",
        maxLevel: 1,
        icon: "icon/property/IconStatusResistance.png",
        parent: "1205103",
      },
      {
        id: "1205209",
        level: 1,
        anchor: "Point17",
        maxLevel: 1,
        icon: "icon/property/IconCriticalChance.png",
        parent: "1205208",
      },
      {
        id: "1205210",
        level: 1,
        anchor: "Point18",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1205208",
      },
    ],
    equipment: {
      lightCone: {
        id: "1205",
        name: "Blade",
        rarity: 5,
        rank: 0,
        level: 80,
        promotion: 6,
        icon: "icon/character/1205.png",
        preview: "image/character_preview/1205.png",
        portrait: "image/character_portrait/1205.png",
        path: {
          id: "Warrior",
          name: "Destruction",
          icon: "icon/path/Destruction.png",
        },
        attributes: [
          {
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 2628.36,
            display: "2628",
            percent: false,
          },
          {
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 1125.4319999999998,
            display: "1125",
            percent: false,
          },
          {
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 815.85,
            display: "815",
            percent: false,
          },
          {
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 97,
            display: "97",
            percent: false,
          },
          {
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.05,
            display: "5.0%",
            percent: true,
          },
          {
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.5,
            display: "50.0%",
            percent: true,
          },
        ],
        properties: [
          {
            type: "HPAddedRatio",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 1.1588800094742382,
            display: "115.8%",
            percent: true,
          },
          {
            type: "CriticalChanceBase",
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.577640003869313,
            display: "57.7%",
            percent: true,
          },
          {
            type: "StatusResistanceBase",
            field: "effect_res",
            name: "Effect RES",
            icon: "icon/property/IconStatusResistance.png",
            value: 0.36352000399493,
            display: "36.3%",
            percent: true,
          },
          {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 815.6776270151604,
            display: "815",
            percent: false,
          },
          {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.19872000301257098,
            display: "19.8%",
            percent: true,
            count: 2,
            step: 4,
          },
          {
            type: "CriticalDamageBase",
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.874800011515622,
            display: "87.4%",
            percent: true,
            count: 1,
            step: 0,
          },
          {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 426.8907070166897,
            display: "426",
            percent: false,
          },
          {
            type: "SpeedDelta",
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 38.83200000715442,
            display: "38",
            percent: false,
            count: 3,
            step: 1,
          },
          {
            type: "BreakDamageAddedRatioBase",
            field: "break_dmg",
            name: "Break Effect",
            icon: "icon/property/IconBreakUp.png",
            value: 0.466560009866952,
            display: "46.6%",
            percent: true,
            count: 4,
            step: 3,
          },
          {
            type: "WindAddedRatio",
            field: "wind_dmg",
            name: "Wind DMG Boost",
            icon: "icon/property/IconWindAddedRatio.png",
            value: 0.38880301429889397,
            display: "38.8%",
            percent: true,
          },
        ],
      },
      relics: [
        {
          id: "61131",
          name: "Disciple's Prosthetic Eye",
          setId: "113",
          setName: "Longevous Disciple",
          rarity: 5,
          level: 15,
          icon: "icon/relic/113_0.png",
          mainAffix: {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 705.6000000101048,
            display: "705",
            percent: false,
          },
          subAffix: [
            {
              type: "HPAddedRatio",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 0.07344000111334101,
              display: "7.3%",
              percent: true,
              count: 2,
              step: 1,
            },
            {
              type: "AttackAddedRatio",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 0.086400001309814,
              display: "8.6%",
              percent: true,
              count: 2,
              step: 4,
            },
            {
              type: "CriticalChanceBase",
              field: "crit_rate",
              name: "CRIT Rate",
              icon: "icon/property/IconCriticalChance.png",
              value: 0.081000001402573,
              display: "8.1%",
              percent: true,
              count: 3,
              step: 1,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.051840000785887,
              display: "5.1%",
              percent: true,
              count: 1,
              step: 0,
            },
          ],
        },
        {
          id: "61132",
          name: "Disciple's Ingenium Hand",
          setId: "113",
          setName: "Longevous Disciple",
          rarity: 5,
          level: 15,
          icon: "icon/relic/113_1.png",
          mainAffix: {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 352.8000000116881,
            display: "352",
            percent: false,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 33.870039001107216,
              display: "33",
              percent: false,
              count: 1,
              step: 0,
            },
            {
              type: "HPAddedRatio",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 0.07344000111334101,
              display: "7.3%",
              percent: true,
              count: 2,
              step: 1,
            },
            {
              type: "SpeedDelta",
              field: "spd",
              name: "SPD",
              icon: "icon/property/IconSpeed.png",
              value: 6.300000000279397,
              display: "6",
              percent: false,
              count: 3,
              step: 1,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.077760001178832,
              display: "7.7%",
              percent: true,
              count: 2,
              step: 2,
            },
          ],
        },
        {
          id: "61133",
          name: "Disciple's Dewy Feather Garb",
          setId: "113",
          setName: "Longevous Disciple",
          rarity: 5,
          level: 15,
          icon: "icon/relic/113_2.png",
          mainAffix: {
            type: "CriticalDamageBase",
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.648000007029628,
            display: "64.8%",
            percent: true,
          },
          subAffix: [
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 16.935019000666216,
              display: "16",
              percent: false,
              count: 1,
              step: 0,
            },
            {
              type: "CriticalChanceBase",
              field: "crit_rate",
              name: "CRIT Rate",
              icon: "icon/property/IconCriticalChance.png",
              value: 0.061560001457111,
              display: "6.1%",
              percent: true,
              count: 2,
              step: 3,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.038880000589416,
              display: "3.8%",
              percent: true,
              count: 1,
              step: 1,
            },
            {
              type: "BreakDamageAddedRatioBase",
              field: "break_dmg",
              name: "Break Effect",
              icon: "icon/property/IconBreakUp.png",
              value: 0.226800004485994,
              display: "22.6%",
              percent: true,
              count: 4,
              step: 3,
            },
          ],
        },
        {
          id: "61134",
          name: "Disciple's Celestial Silk Sandals",
          setId: "113",
          setName: "Longevous Disciple",
          rarity: 5,
          level: 15,
          icon: "icon/relic/113_3.png",
          mainAffix: {
            type: "SpeedDelta",
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 25.03200000547804,
            display: "25",
            percent: false,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 76.20758800394833,
              display: "76",
              percent: false,
              count: 2,
              step: 2,
            },
            {
              type: "AttackAddedRatio",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 0.077760001178832,
              display: "7.7%",
              percent: true,
              count: 2,
              step: 2,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.07344000111334101,
              display: "7.3%",
              percent: true,
              count: 2,
              step: 1,
            },
            {
              type: "BreakDamageAddedRatioBase",
              field: "break_dmg",
              name: "Break Effect",
              icon: "icon/property/IconBreakUp.png",
              value: 0.116640002466738,
              display: "11.6%",
              percent: true,
              count: 2,
              step: 2,
            },
          ],
        },
        {
          id: "63095",
          name: "Taikiyan Laser Stadium",
          setId: "309",
          setName: "Rutilant Arena",
          rarity: 5,
          level: 15,
          icon: "icon/relic/309_0.png",
          mainAffix: {
            type: "WindAddedRatio",
            field: "wind_dmg",
            name: "Wind DMG Boost",
            icon: "icon/property/IconWindAddedRatio.png",
            value: 0.38880301429889397,
            display: "38.8%",
            percent: true,
          },
          subAffix: [
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 16.935019000666216,
              display: "16",
              percent: false,
              count: 1,
              step: 0,
            },
            {
              type: "CriticalChanceBase",
              field: "crit_rate",
              name: "CRIT Rate",
              icon: "icon/property/IconCriticalChance.png",
              value: 0.055080001009628995,
              display: "5.5%",
              percent: true,
              count: 2,
              step: 1,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.174960003700107,
              display: "17.4%",
              percent: true,
              count: 3,
              step: 3,
            },
            {
              type: "BreakDamageAddedRatioBase",
              field: "break_dmg",
              name: "Break Effect",
              icon: "icon/property/IconBreakUp.png",
              value: 0.12312000291422,
              display: "12.3%",
              percent: true,
              count: 2,
              step: 3,
            },
          ],
        },
        {
          id: "63096",
          name: "Taikiyan's Arclight Race Track",
          setId: "309",
          setName: "Rutilant Arena",
          rarity: 5,
          level: 15,
          icon: "icon/relic/309_1.png",
          mainAffix: {
            type: "HPAddedRatio",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 0.43200000724755605,
            display: "43.2%",
            percent: true,
          },
          subAffix: [
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 40.22066900366917,
              display: "40",
              percent: false,
              count: 2,
              step: 3,
            },
            {
              type: "AttackAddedRatio",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 0.034560000523925,
              display: "3.4%",
              percent: true,
              count: 1,
              step: 0,
            },
            {
              type: "SpeedDelta",
              field: "spd",
              name: "SPD",
              icon: "icon/property/IconSpeed.png",
              value: 7.500000001396985,
              display: "7",
              percent: false,
              count: 3,
              step: 5,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.07344000111334101,
              display: "7.3%",
              percent: true,
              count: 2,
              step: 1,
            },
          ],
        },
      ],
    },
  },
  {
    id: "1213",
    name: "Dan Heng • Imbibitor Lunae",
    rarity: 5,
    rank: 0,
    level: 80,
    promotion: 6,
    path: {
      id: "Warrior",
      name: "Destruction",
      icon: "icon/path/Destruction.png",
    },
    element: {
      id: "Imaginary",
      name: "Imaginary",
      color: "#F4D258",
      icon: "icon/element/Imaginary.png",
    },
    skills: [
      {
        id: "121301",
        name: "Beneficent Lotus",
        level: 5,
        maxLevel: 9,
        element: {
          id: "Imaginary",
          name: "Imaginary",
          color: "#F4D258",
          icon: "icon/element/Imaginary.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc: "Deals minor Imaginary DMG to a single enemy.",
        desc: "Uses a 2-hit attack and deals Imaginary DMG equal to 90% of Dan Heng • Imbibitor Lunae's ATK to a single enemy target.",
        icon: "icon/skill/1213_basic_atk.png",
      },
      {
        id: "121302",
        name: "Dracore Libre",
        level: 6,
        maxLevel: 15,
        element: null,
        type: "BPSkill",
        typeText: "Skill",
        effect: "Enhance",
        effectText: "Enhance",
        simpleDesc:
          "Enhances the Basic ATK Beneficent Lotus to Transcendence, Divine Spear, or Fulgurant Leap.",
        desc: "Enhances Basic ATK. Enhancements may be applied up to 3 times consecutively. Using this ability does not consume Skill Points and is not considered as using a Skill.\nEnhanced once, Beneficent Lotus becomes Transcendence.\nEnhanced twice, Beneficent Lotus becomes Divine Spear.\nEnhanced thrice, Beneficent Lotus becomes Fulgurant Leap.\nWhen using Divine Spear or Fulgurant Leap, starting from the fourth hit, 1 stack of Outroar is gained before every hit. Each stack of Outroar increases Dan Heng • Imbibitor Lunae's CRIT DMG by 9.0%, for a max of 4 stacks. These stacks last until the end of his turn.",
        icon: "icon/skill/1213_skill.png",
      },
      {
        id: "121303",
        name: "Azure's Aqua Ablutes All",
        level: 6,
        maxLevel: 15,
        element: {
          id: "Imaginary",
          name: "Imaginary",
          color: "#F4D258",
          icon: "icon/element/Imaginary.png",
        },
        type: "Ultra",
        typeText: "Ultimate",
        effect: "Blast",
        effectText: "Blast",
        simpleDesc:
          "Deals massive Imaginary DMG to a single enemy, deals Imaginary DMG to adjacent targets, and gains 2 Squama Sacrosancta, which can offset Dan Heng • Imbibitor Lunae's consumption of skill points. Consuming Squama Sacrosancta is considered equivalent to consuming skill points.",
        desc: "Uses a 3-hit attack and deals Imaginary DMG equal to 240% of Dan Heng • Imbibitor Lunae's ATK to a single enemy target. At the same time, deals Imaginary DMG equal to 112% of Dan Heng • Imbibitor Lunae's ATK to adjacent targets. Then, obtains 2 Squama Sacrosancta.\nIt's possible to hold up to 3 Squama Sacrosancta, which can be used to offset Dan Heng • Imbibitor Lunae's consumption of skill points. Consuming Squama Sacrosancta is considered equivalent to consuming skill points.",
        icon: "icon/skill/1213_ultimate.png",
      },
      {
        id: "121304",
        name: "Righteous Heart",
        level: 7,
        maxLevel: 15,
        element: null,
        type: "Talent",
        typeText: "Talent",
        effect: "Enhance",
        effectText: "Enhance",
        simpleDesc:
          "Increases DMG for every hit dealt. This effect is stackable and lasts until the end of this character's turn.",
        desc: "After each hit dealt during an attack, Dan Heng • Imbibitor Lunae gains 1 stack of Righteous Heart, increasing his DMG by 8.1%. This effect can stack up to 6 time(s), lasting until the end of his turn.",
        icon: "icon/skill/1213_talent.png",
      },
      {
        id: "121306",
        name: "121306",
        level: 0,
        maxLevel: 1,
        element: {
          id: "Imaginary",
          name: "Imaginary",
          color: "#F4D258",
          icon: "icon/element/Imaginary.png",
        },
        type: "MazeNormal",
        typeText: "",
        effect: "MazeAttack",
        effectText: "",
        simpleDesc: "",
        desc: "",
        icon: "icon/skill/1213_basic_atk.png",
      },
      {
        id: "121307",
        name: "Heaven-Quelling Prismadrakon",
        level: 1,
        maxLevel: 1,
        element: null,
        type: "Maze",
        typeText: "Technique",
        effect: "Enhance",
        effectText: "Enhance",
        simpleDesc:
          "Enters the Leaping Dragon state. Attacking will cause this character to move forward rapidly for a set distance and attack all enemies touched. After entering combat via attacking enemies, deals Imaginary DMG to all enemies, and gains 1 Squama Sacrosancta.",
        desc: "After using his Technique, Dan Heng • Imbibitor Lunae enters the Leaping Dragon state for 20 seconds. While in the Leaping Dragon state, using his attack enables him to move forward rapidly for a set distance, attacking all enemies he touches and blocking all incoming attacks. After entering combat via attacking enemies in the Leaping Dragon state, Dan Heng • Imbibitor Lunae deals Imaginary DMG equal to 120% of his ATK to all enemies, and gains 1 Squama Sacrosancta.",
        icon: "icon/skill/1213_technique.png",
      },
      {
        id: "121308",
        name: "Transcendence",
        level: 5,
        maxLevel: 9,
        element: {
          id: "Imaginary",
          name: "Imaginary",
          color: "#F4D258",
          icon: "icon/element/Imaginary.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "SingleAttack",
        effectText: "Single Target",
        simpleDesc:
          "Unleashes 3 hits in this attack and deals Imaginary DMG to a single enemy target.",
        desc: "Uses a 3-hit attack and deals Imaginary DMG equal to 234% of Dan Heng • Imbibitor Lunae's ATK to a single enemy target.",
        icon: "icon/skill/1213_basic_atk.png",
      },
      {
        id: "121309",
        name: "121309",
        level: 0,
        maxLevel: 1,
        element: null,
        type: "BPSkill",
        typeText: "Skill",
        effect: "Enhance",
        effectText: "Cancel",
        simpleDesc: "",
        desc: "",
        icon: "icon/skill/1213_skill.png",
      },
      {
        id: "121310",
        name: "Divine Spear",
        level: 5,
        maxLevel: 9,
        element: {
          id: "Imaginary",
          name: "Imaginary",
          color: "#F4D258",
          icon: "icon/element/Imaginary.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "Blast",
        effectText: "Blast",
        simpleDesc:
          "Unleashes 5 hits in this attack to deal massive Imaginary DMG to a single enemy and minor Imaginary DMG to adjacent targets. For each of the last 2 hits, obtains 1 stack of Outroar just before the hit, increasing CRIT DMG.",
        desc: "Uses a 5-hit attack and deals Imaginary DMG equal to 190% of Dan Heng • Imbibitor Lunae's ATK to a single enemy target. From the fourth hit onward, simultaneously deals Imaginary DMG equal to 30% of Dan Heng • Imbibitor Lunae's ATK to adjacent targets.",
        icon: "icon/skill/1213_basic_atk.png",
      },
      {
        id: "121312",
        name: "Fulgurant Leap",
        level: 5,
        maxLevel: 9,
        element: {
          id: "Imaginary",
          name: "Imaginary",
          color: "#F4D258",
          icon: "icon/element/Imaginary.png",
        },
        type: "Normal",
        typeText: "Basic ATK",
        effect: "Blast",
        effectText: "Blast",
        simpleDesc:
          "Unleashes 7 hits in this attack to deal massive Imaginary DMG to a single enemy and Imaginary DMG to adjacent targets. For each of the last 4 hits, obtains 1 stack of Outroar just before the hit, increasing CRIT DMG.",
        desc: "Uses a 7-hit attack and deals Imaginary DMG equal to 250% of Dan Heng • Imbibitor Lunae's ATK to a single enemy target. From the fourth hit onward, simultaneously deal Imaginary DMG equal to 90% of Dan Heng • Imbibitor Lunae's ATK to adjacent targets.",
        icon: "icon/skill/1213_basic_atk.png",
      },
    ],
    skillTrees: [
      {
        id: "1213001",
        level: 5,
        anchor: "Point01",
        maxLevel: 6,
        icon: "icon/skill/1213_basic_atk.png",
        parent: null,
      },
      {
        id: "1213002",
        level: 6,
        anchor: "Point02",
        maxLevel: 10,
        icon: "icon/skill/1213_skill.png",
        parent: null,
      },
      {
        id: "1213003",
        level: 6,
        anchor: "Point03",
        maxLevel: 10,
        icon: "icon/skill/1213_ultimate.png",
        parent: null,
      },
      {
        id: "1213004",
        level: 7,
        anchor: "Point04",
        maxLevel: 10,
        icon: "icon/skill/1213_talent.png",
        parent: null,
      },
      {
        id: "1213007",
        level: 1,
        anchor: "Point05",
        maxLevel: 1,
        icon: "icon/skill/1213_technique.png",
        parent: null,
      },
      {
        id: "1213101",
        level: 1,
        anchor: "Point06",
        maxLevel: 1,
        icon: "icon/skill/1213_skilltree1.png",
        parent: null,
      },
      {
        id: "1213102",
        level: 1,
        anchor: "Point07",
        maxLevel: 1,
        icon: "icon/skill/1213_skilltree2.png",
        parent: null,
      },
      {
        id: "1213103",
        level: 1,
        anchor: "Point08",
        maxLevel: 1,
        icon: "icon/skill/1213_skilltree3.png",
        parent: null,
      },
      {
        id: "1213201",
        level: 1,
        anchor: "Point09",
        maxLevel: 1,
        icon: "icon/property/IconImaginaryAddedRatio.png",
        parent: null,
      },
      {
        id: "1213202",
        level: 1,
        anchor: "Point10",
        maxLevel: 1,
        icon: "icon/property/IconCriticalChance.png",
        parent: "1213101",
      },
      {
        id: "1213203",
        level: 1,
        anchor: "Point11",
        maxLevel: 1,
        icon: "icon/property/IconImaginaryAddedRatio.png",
        parent: "1213202",
      },
      {
        id: "1213204",
        level: 1,
        anchor: "Point12",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1213203",
      },
      {
        id: "1213205",
        level: 1,
        anchor: "Point13",
        maxLevel: 1,
        icon: "icon/property/IconImaginaryAddedRatio.png",
        parent: "1213102",
      },
      {
        id: "1213206",
        level: 1,
        anchor: "Point14",
        maxLevel: 1,
        icon: "icon/property/IconCriticalChance.png",
        parent: "1213205",
      },
      {
        id: "1213207",
        level: 1,
        anchor: "Point15",
        maxLevel: 1,
        icon: "icon/property/IconImaginaryAddedRatio.png",
        parent: "1213206",
      },
      {
        id: "1213208",
        level: 1,
        anchor: "Point16",
        maxLevel: 1,
        icon: "icon/property/IconMaxHP.png",
        parent: "1213103",
      },
      {
        id: "1213209",
        level: 1,
        anchor: "Point17",
        maxLevel: 1,
        icon: "icon/property/IconCriticalChance.png",
        parent: "1213208",
      },
      {
        id: "1213210",
        level: 1,
        anchor: "Point18",
        maxLevel: 1,
        icon: "icon/property/IconImaginaryAddedRatio.png",
        parent: "1213208",
      },
    ],
    equipment: {
      lightCone: {
        id: "1213",
        name: "Dan Heng • Imbibitor Lunae",
        rarity: 5,
        rank: 0,
        level: 80,
        promotion: 6,
        icon: "icon/character/1213.png",
        preview: "image/character_preview/1213.png",
        portrait: "image/character_portrait/1213.png",
        path: {
          id: "Warrior",
          name: "Destruction",
          icon: "icon/path/Destruction.png",
        },
        attributes: [
          {
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 2300.2560000000003,
            display: "2300",
            percent: false,
          },
          {
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 1227.7440000000001,
            display: "1227",
            percent: false,
          },
          {
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 760.725,
            display: "760",
            percent: false,
          },
          {
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 102,
            display: "102",
            percent: false,
          },
          {
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.05,
            display: "5.0%",
            percent: true,
          },
          {
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.5,
            display: "50.0%",
            percent: true,
          },
        ],
        properties: [
          {
            type: "ImaginaryAddedRatio",
            field: "imaginary_dmg",
            name: "Imaginary DMG Boost",
            icon: "icon/property/IconImaginaryAddedRatio.png",
            value: 0.7128030142988939,
            display: "71.2%",
            percent: true,
          },
          {
            type: "CriticalChanceBase",
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.637400000868361,
            display: "63.7%",
            percent: true,
          },
          {
            type: "HPAddedRatio",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 0.307360003143553,
            display: "30.7%",
            percent: true,
          },
          {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 959.6252930213232,
            display: "959",
            percent: false,
          },
          {
            type: "CriticalDamageBase",
            field: "crit_dmg",
            name: "CRIT DMG",
            icon: "icon/property/IconCriticalDamage.png",
            value: 0.34344000695273197,
            display: "34.3%",
            percent: true,
            count: 2,
            step: 2,
          },
          {
            type: "StatusResistanceBase",
            field: "effect_res",
            name: "Effect RES",
            icon: "icon/property/IconStatusResistance.png",
            value: 0.276480004191403,
            display: "27.6%",
            percent: true,
            count: 2,
            step: 2,
          },
          {
            type: "BreakDamageAddedRatioBase",
            field: "break_dmg",
            name: "Break Effect",
            icon: "icon/property/IconBreakUp.png",
            value: 0.401760008186101,
            display: "40.1%",
            percent: true,
            count: 4,
            step: 3,
          },
          {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 426.8907070166897,
            display: "426",
            percent: false,
          },
          {
            type: "DefenceAddedRatio",
            field: "def",
            name: "DEF",
            icon: "icon/property/IconDefence.png",
            value: 0.107999999541788,
            display: "10.7%",
            percent: true,
            count: 1,
            step: 2,
          },
          {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.907200015150019,
            display: "90.7%",
            percent: true,
            count: 1,
            step: 2,
          },
          {
            type: "SpeedDelta",
            field: "spd",
            name: "SPD",
            icon: "icon/property/IconSpeed.png",
            value: 12.400000002235176,
            display: "12",
            percent: false,
            count: 2,
            step: 4,
          },
          {
            type: "StatusProbabilityBase",
            field: "effect_hit",
            name: "Effect Hit Rate",
            icon: "icon/property/IconStatusProbability.png",
            value: 0.034560000523925,
            display: "3.4%",
            percent: true,
            count: 1,
            step: 0,
          },
        ],
      },
      relics: [
        {
          id: "61121",
          name: "Wastelander's Breathing Mask",
          setId: "112",
          setName: "Wastelander of Banditry Desert",
          rarity: 5,
          level: 15,
          icon: "icon/relic/112_0.png",
          mainAffix: {
            type: "HPDelta",
            field: "hp",
            name: "HP",
            icon: "icon/property/IconMaxHP.png",
            value: 705.6000000101048,
            display: "705",
            percent: false,
          },
          subAffix: [
            {
              type: "CriticalChanceBase",
              field: "crit_rate",
              name: "CRIT Rate",
              icon: "icon/property/IconCriticalChance.png",
              value: 0.025920000392944,
              display: "2.5%",
              percent: true,
              count: 1,
              step: 0,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.116640002466738,
              display: "11.6%",
              percent: true,
              count: 2,
              step: 2,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.077760001178832,
              display: "7.7%",
              percent: true,
              count: 2,
              step: 2,
            },
            {
              type: "BreakDamageAddedRatioBase",
              field: "break_dmg",
              name: "Break Effect",
              icon: "icon/property/IconBreakUp.png",
              value: 0.226800004485994,
              display: "22.6%",
              percent: true,
              count: 4,
              step: 3,
            },
          ],
        },
        {
          id: "61122",
          name: "Wastelander's Desert Terminal",
          setId: "112",
          setName: "Wastelander of Banditry Desert",
          rarity: 5,
          level: 15,
          icon: "icon/relic/112_1.png",
          mainAffix: {
            type: "AttackDelta",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 352.8000000116881,
            display: "352",
            percent: false,
          },
          subAffix: [
            {
              type: "HPAddedRatio",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 0.120960001833739,
              display: "12.0%",
              percent: true,
              count: 3,
              step: 4,
            },
            {
              type: "DefenceAddedRatio",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 0.053999999770894,
              display: "5.3%",
              percent: true,
              count: 1,
              step: 2,
            },
            {
              type: "CriticalChanceBase",
              field: "crit_rate",
              name: "CRIT Rate",
              icon: "icon/property/IconCriticalChance.png",
              value: 0.087480001850055,
              display: "8.7%",
              percent: true,
              count: 3,
              step: 3,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.043200000654907,
              display: "4.3%",
              percent: true,
              count: 1,
              step: 2,
            },
          ],
        },
        {
          id: "61123",
          name: "Wastelander's Friar Robe",
          setId: "112",
          setName: "Wastelander of Banditry Desert",
          rarity: 5,
          level: 15,
          icon: "icon/relic/112_2.png",
          mainAffix: {
            type: "CriticalChanceBase",
            field: "crit_rate",
            name: "CRIT Rate",
            icon: "icon/property/IconCriticalChance.png",
            value: 0.32399999862536205,
            display: "32.3%",
            percent: true,
          },
          subAffix: [
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 40.22066900366917,
              display: "40",
              percent: false,
              count: 2,
              step: 3,
            },
            {
              type: "AttackAddedRatio",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 0.043200000654907,
              display: "4.3%",
              percent: true,
              count: 1,
              step: 2,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.06912000104785,
              display: "6.9%",
              percent: true,
              count: 2,
              step: 0,
            },
            {
              type: "BreakDamageAddedRatioBase",
              field: "break_dmg",
              name: "Break Effect",
              icon: "icon/property/IconBreakUp.png",
              value: 0.174960003700107,
              display: "17.4%",
              percent: true,
              count: 3,
              step: 3,
            },
          ],
        },
        {
          id: "61124",
          name: "Wastelander's Powered Greaves",
          setId: "112",
          setName: "Wastelander of Banditry Desert",
          rarity: 5,
          level: 15,
          icon: "icon/relic/112_3.png",
          mainAffix: {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.43200000724755605,
            display: "43.2%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 105.8438720041886,
              display: "105",
              percent: false,
              count: 3,
              step: 1,
            },
            {
              type: "SpeedDelta",
              field: "spd",
              name: "SPD",
              icon: "icon/property/IconSpeed.png",
              value: 5.200000001117588,
              display: "5",
              percent: false,
              count: 2,
              step: 4,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.103680001571774,
              display: "10.3%",
              percent: true,
              count: 2,
              step: 0,
            },
            {
              type: "StatusProbabilityBase",
              field: "effect_hit",
              name: "Effect Hit Rate",
              icon: "icon/property/IconStatusProbability.png",
              value: 0.034560000523925,
              display: "3.4%",
              percent: true,
              count: 1,
              step: 0,
            },
          ],
        },
        {
          id: "63095",
          name: "Taikiyan Laser Stadium",
          setId: "309",
          setName: "Rutilant Arena",
          rarity: 5,
          level: 15,
          icon: "icon/relic/309_0.png",
          mainAffix: {
            type: "ImaginaryAddedRatio",
            field: "imaginary_dmg",
            name: "Imaginary DMG Boost",
            icon: "icon/property/IconImaginaryAddedRatio.png",
            value: 0.38880301429889397,
            display: "38.8%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 76.20758800394833,
              display: "76",
              percent: false,
              count: 2,
              step: 2,
            },
            {
              type: "HPAddedRatio",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 0.086400001309814,
              display: "8.6%",
              percent: true,
              count: 2,
              step: 4,
            },
            {
              type: "DefenceAddedRatio",
              field: "def",
              name: "DEF",
              icon: "icon/property/IconDefence.png",
              value: 0.053999999770894,
              display: "5.3%",
              percent: true,
              count: 1,
              step: 2,
            },
            {
              type: "SpeedDelta",
              field: "spd",
              name: "SPD",
              icon: "icon/property/IconSpeed.png",
              value: 7.200000001117588,
              display: "7",
              percent: false,
              count: 3,
              step: 4,
            },
          ],
        },
        {
          id: "63096",
          name: "Taikiyan's Arclight Race Track",
          setId: "309",
          setName: "Rutilant Arena",
          rarity: 5,
          level: 15,
          icon: "icon/relic/309_1.png",
          mainAffix: {
            type: "AttackAddedRatio",
            field: "atk",
            name: "ATK",
            icon: "icon/property/IconAttack.png",
            value: 0.43200000724755605,
            display: "43.2%",
            percent: true,
          },
          subAffix: [
            {
              type: "HPDelta",
              field: "hp",
              name: "HP",
              icon: "icon/property/IconMaxHP.png",
              value: 71.97383300308138,
              display: "71",
              percent: false,
              count: 2,
              step: 1,
            },
            {
              type: "AttackDelta",
              field: "atk",
              name: "ATK",
              icon: "icon/property/IconAttack.png",
              value: 33.87003800133243,
              display: "33",
              percent: false,
              count: 2,
              step: 0,
            },
            {
              type: "CriticalDamageBase",
              field: "crit_dmg",
              name: "CRIT DMG",
              icon: "icon/property/IconCriticalDamage.png",
              value: 0.12312000291422,
              display: "12.3%",
              percent: true,
              count: 2,
              step: 3,
            },
            {
              type: "StatusResistanceBase",
              field: "effect_res",
              name: "Effect RES",
              icon: "icon/property/IconStatusResistance.png",
              value: 0.086400001309814,
              display: "8.6%",
              percent: true,
              count: 2,
              step: 4,
            },
          ],
        },
      ],
    },
  },
];
